...
class MyDecentralized:
    ...
    def train(self, trainX, trainY):
        ...
        # Adjust values of trainY
        unique_vals = np.unique(trainY)
        if np.array_equal(unique_vals, np.array([0,3,4])):
            # 3 becomes 1, 4 becomes 2
            self.adjust_labels = True
            forward_map = {0: 0, 3: 1, 4: 2}
            trainY = np.vectorize(forward_map.get)(trainY)

        # Decision variables
        W = cp.Variable((K, M))   # weight
        b = cp.Variable(K)        # bias
        u = cp.Variable((K, M))   # L1 linearization auxiliaries for W
        slack = cp.Variable((N, self.K))

        # Objective
        # scores = trainX @ W.T + b
        C = np.eye(K)[trainY] # one hot encoding

        # Objective
        objective = cp.Minimize(cp.sum(slack) + self.reg*cp.sum(u))

        # Constraints
        constraints = []
        for i in range(N):
            for k in range(self.K):
                y_binary = 1 if C[i, k] == 1 else -1
                constraints.append(y_binary * (W[k] @ trainX[i] + b[k]) >= 1 - slack[i, k])
                constraints.append(slack[i, k] >= 0)
        constraints += [u >= W]
        constraints += [u >= -W]
    ...
    def predict(self, testX):
        ...
        '''
        predY = np.argmax(testX @ self.W.T + self.b, axis=1)
        if self.adjust_labels:
            backward_map = {0: 0, 1: 3, 2: 4}
            predY = np.vectorize(backward_map.get)(predY)
        return predY
...
class MyFeatureCompression:
    ...
    def run_LP(self, X, centroids, N, n_c):
        ...
        # Objective
        objective = cp.Minimize(cp.sum(cp.multiply(u, dists)))
        # Constraints
        constraints = [cp.sum(u[i, :]) == 1 for i in range(N)]
        # Solve
        prob = cp.Problem(objective, constraints)
        prob.solve(solver=cp.MOSEK, verbose=True)
        if u.value is None:
    ...
    def run_centralized(self, trainX, trainY, valX, valY, testX, testY, B_tot_list):
        ...
        """
        # one codebook
        num_centroids = [2**i for i in B_tot_list]
        test_accuracy = []
        N, M = trainX.shape
        # K = self.K
        print("Computing centroids...")
        for n_c in num_centroids:
            # Clustering Loop
            stop = False
            centroids_prev = None
            while not stop:
                if centroids_prev is None:
                    idx = np.random.choice(trainX.shape[0], size=n_c, replace=False)
                    centroids = trainX[idx]
                else:
                    sum_clusters = self.u_cluster.T @ trainX
                    counts = self.u_cluster.sum(axis=0)
                    # avoid division by 0
                    counts_safe = counts.copy()
                    counts_safe[counts_safe == 0] = 1
                    centroids = sum_clusters / counts_safe[:, None] # [n_c, M]
                    empty = (counts == 0)
                    if empty.any():
                        # reinitialize to a random train point
                        centroids[empty] = trainX[np.random.choice(trainX.shape[0], size=empty.sum())]
            ...
            quantized = self.u_cluster@centroids
        ...
        result = {'B_tot': B_tot_list, 'test_accuracy': test_accuracy}
        return result
